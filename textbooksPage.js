...
function generateChecklist() {
  ...
  const clearRectangle = document.querySelector(
    "#column_selected_Classes .clear_rectangle"
  );
  const costsContainer = document.querySelector(
    "#column_Costs .clear_rectangle"
  );
  ...
  for (const key in checklistItems) {
    if (checklistItems.hasOwnProperty(key)) {
      ...
      checkbox.id = `checkbox-${item.courseName.replace(
        /\s+/g,
        ""
      )}-${item.teachers.replace(/\s+/g, "")}`;
      ...
      function handleClassSelection(checkbox, item) {
        if (selectedCourses.has(item.courseName + " " + item.teachers)) {
          //this is called when you are removing a class

          console.log(
            "this checkbox is currently checked: " + checkbox.checked
          );

          checkbox.checked = false; // Uncheck the checkbox

          console.log(
            "this checkbox is not checked anymore: " + checkbox.checked
          );
          ...
          item.textbooks.forEach((textbook) => {
            ...
            handleCourseSelection(
              item.courseName + " " + item.teachers,
              textbook.price,
              false,
              textbook.title
            ); // Manually call unselect handler
            ...
            totalCostElement.textContent = `Total Cost: ${totalCost.toFixed(
              2
            )}`;
          ...
          selectedCourseItem.textContent =
            item.courseName + " " + item.teachers;
        ...
        complete: function (results) {
          ...
          textbookData1.forEach((textbook) => {
            ...
            if (bookInfo.courseName.indexOf("[") > 0) {
              ...
              bookInfo.courseName =
                bookInfo.courseName.substring(0, indexOne) +
                bookInfo.courseName.substring(indexTwo + 1);
...
function updateGlobalCostDisplay() {
  ...
  const globalCost = parseFloat(
    localStorage.getItem("globalCost") || "0"
  ).toFixed(2);
...
function alterTotalCost(textbookPrice, isAdd) {
  ...
  globalCost =
    parseFloat(localStorage.getItem("globalCost") || "0") +
    (isAdd ? textbookPrice : -textbookPrice);
...
function saveSelectedCourses() {
  const selectedCoursesData = Array.from(selectedCourses).map(courseKey => {
      return {
          key: courseKey,
          cost: textbookData.find(textbook => `${textbook.courseName}_${textbook.teachers}` === courseKey)?.price || 0
      };
  });
  localStorage.setItem('selectedCourses', JSON.stringify(selectedCoursesData));
}
